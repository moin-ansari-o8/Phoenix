...
class PhoenixPluginIntegration:
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Plugin integration initialized with {len(self.registry.plugins)} plugins"
            )
...
def demo_plugins():
    ...
    try:
        ...
        for plugin_name, actions in all_actions.items():
            print(f"{plugin_name}:")
            for action, desc in list(actions.items())[:5]:  # First 5
                print(f"    {action}: {desc}")
            if len(actions) > 5:
        ...
        print("Make sure plugin_temp package is in PYTHONPATH")
